...
class TradeIndiaProvider(BaseDiscoveryProvider):
    ...
    async def discover(self, keyword: str, location: str, limit: int = 20, website_filter: str = "all", **kwargs) -> List[dict]:
        """TradeIndia provider extracting real live trader & exporter directory listings."""
        clean_keyword = keyword.strip()
        clean_location = location.strip()
        logger.info(f"Extracting real TradeIndia directory leads for '{clean_keyword}' in '{clean_location}'")
